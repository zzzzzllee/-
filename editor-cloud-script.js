(() => {
  ...
  const imageAdjustmentValue = key => { const saved=content?.imageAdjustments?.[key]||{};const legacy=clampImageScale(saved.scale);return {x:Number(saved.x)||0,y:Number(saved.y)||0,scale:legacy,scaleX:clampImageScale(saved.scaleX??legacy),scaleY:clampImageScale(saved.scaleY??legacy),crop:saved.cropEnabled===true?normalizeCrop(saved.crop):null}; };
  const normalizedImageAdjustment = value => { const legacy=clampImageScale(value?.scale);const result={x:Math.round(Number(value?.x)||0),y:Math.round(Number(value?.y)||0),scaleX:Number(clampImageScale(value?.scaleX??legacy).toFixed(3)),scaleY:Number(clampImageScale(value?.scaleY??legacy).toFixed(3))};const crop=normalizeCrop(value?.crop);if(crop&&value?.cropEnabled===true){result.crop=crop;result.cropEnabled=true;}return result; };
  const applyCropStyle = (img,crop) => { if(!img)return;const value=normalizeCrop(crop);['width','height','max-width','max-height','left','top','position','object-fit','object-position'].forEach(name=>img.style.removeProperty(name));if(!value){img.removeAttribute('data-image-cropped');img.style.setProperty('--image-crop-x','0%');img.style.setProperty('--image-crop-y','0%');img.style.setProperty('--image-crop-scale-x','1');img.style.setProperty('--image-crop-scale-y','1');img.style.setProperty('object-fit','contain','important');img.style.setProperty('object-position','center','important');return;}img.dataset.imageCropped='1';img.style.setProperty('--image-crop-x',(-value.left*100/value.width)+'%');img.style.setProperty('--image-crop-y',(-value.top*100/value.height)+'%');img.style.setProperty('--image-crop-scale-x',String(1/value.width));img.style.setProperty('--image-crop-scale-y',String(1/value.height));img.style.setProperty('object-fit','fill','important');img.style.setProperty('object-position','center','important'); };
...
doc.addEventListener('pointermove',event=>{
  ...
  const ensureVideoUpload = () => { if (document.querySelector('[data-video-upload]')) return; const urlField = document.querySelector('[data-path="video.url"]')?.closest('.field'); if (!urlField) return; const wrapper = document.createElement('div'); wrapper.className = 'field full'; wrapper.innerHTML = '<label>上传宣传片文件</label><input type="file" accept="video/mp4,video/webm,video/ogg" data-video-upload><small>选择视频后点击“立即保存到云端”；云端使用分片上传。若出现 413，请把视频压缩到 50MB 以内。</small>'; urlField.parentElement?.appendChild(wrapper); wrapper.querySelector('[data-video-upload]').addEventListener('change', event => { const file = event.target.files?.[0]; if (!file) return; if (!/^video\/(mp4|webm|ogg)$/i.test(file.type || '')) { status('宣传片格式不支持，请选择 MP4、WebM 或 OGG 视频。', true); event.target.value = ''; return; } const sizeMb = (file.size / 1024 / 1024).toFixed(1); pendingVideo = { file }; scheduleDraft(); status('已选择宣传片（' + sizeMb + 'MB），请点击“立即保存到云端”；云端会使用分片上传。'); }); };
