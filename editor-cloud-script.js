(() => {
  ...
  const visual = { selected: null, selectedImage: null, doc: null, drag: null, pendingDrag: null, imageDrag: null, imageResize: null, cropOverlay: null, suppressClickUntil: 0, mode: 'browse' };
  ...
  const imageAdjustmentValue = key => { const saved=content?.imageAdjustments?.[key]||{};const legacy=clampImageScale(saved.scale);return {x:Number(saved.x)||0,y:Number(saved.y)||0,scale:legacy,scaleX:clampImageScale(saved.scaleX??legacy),scaleY:clampImageScale(saved.scaleY??legacy)}; };
  const normalizedImageAdjustment = value => { const legacy=clampImageScale(value?.scale);return {x:Math.round(Number(value?.x)||0),y:Math.round(Number(value?.y)||0),scaleX:Number(clampImageScale(value?.scaleX??legacy).toFixed(3)),scaleY:Number(clampImageScale(value?.scaleY??legacy).toFixed(3))}; };
  const updateImageAdjustmentStyle = (img,value) => { if(!img)return;const normalized=normalizedImageAdjustment(value);img.style.setProperty('--image-x',normalized.x+'px');img.style.setProperty('--image-y',normalized.y+'px');img.style.setProperty('--image-scale-x',String(normalized.scaleX));img.style.setProperty('--image-scale-y',String(normalized.scaleY));img.style.setProperty('--image-scale',String((normalized.scaleX+normalized.scaleY)/2)); };
...
doc.addEventListener('pointermove',event=>{
          ...
          if(imageResize){
            ...
            const current=imageAdjustmentValue(imageResize.key);const value={x:current.x,y:current.y,scaleX,scaleY};content.imageAdjustments=content.imageAdjustments||{};content.imageAdjustments[imageResize.key]=normalizedImageAdjustment(value);updateImageAdjustmentStyle(imageResize.img,value);updateCropOverlay();return;
          }
         ...
         if(imageDrag){
           ...
           if(event.cancelable)event.preventDefault();
           const value={x:Math.round(imageDrag.x+event.clientX-imageDrag.startX),y:Math.round(imageDrag.y+event.clientY-imageDrag.startY),scaleX:imageDrag.scaleX,scaleY:imageDrag.scaleY};
